      ...
      for(var item in cart) {
        if(cart[item].id === id) {
          ...
          if(cart[item].count === 0) {
            // Hide the qtyBoxCmp for the specific item
            $('.qtyBoxCmp[data-id="' + cart[item].id + '"]').hide(500);

            // Show the Add to Cart button for the specific item
            $('.add-to-cart[data-id="' + cart[item].id + '"]').show(500);
